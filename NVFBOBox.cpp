...
bool NVFBOBox::Initialize(int w, int h, float ssfact, int depthSamples_, int coverageSamples_, int tilesW, int tilesH, bool bOneFBOPerTile_)
{
	...
	downsampling[2].addFragmentShaderFromString(
		...
		"	vec4 tap11 = texture2D(texImage, gl_TexCoord[0].xy + texelSize * vec2(  0.4,  0.9 ));\n"
		"	vec4 tap21 = texture2D(texImage, gl_TexCoord[0].xy + texelSize * vec2( -0.4, -0.9 ));\n"
		"	vec4 tap31 = texture2D(texImage, gl_TexCoord[0].xy + texelSize * vec2( -0.9,  0.4 ));\n"
		"	vec4 tap41 = texture2D(texImage, gl_TexCoord[0].xy + texelSize * vec2(  0.9, -0.4 ));\n"
